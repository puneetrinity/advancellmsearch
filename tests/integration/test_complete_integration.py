...
async def mock_model_manager():
    ...
    def mock_generate(model_name, prompt, **kwargs):
        ...
        if "hello" in prompt.lower() or "hi" in prompt.lower():
            response = "Hello! I'm here to help you with any questions you "
    "have."
            intent_response = "conversation"
        elif "code" in prompt.lower() or "function" in prompt.lower():
            response = "Here's how you can write that function in "
                "Python:\n\n```python\ndef example():\n    pass\n```"
            intent_response = "code"
        elif "explain" in prompt.lower() or "what is" in prompt.lower():
            response = "Let me explain that concept for you. This is a "
            ...
            response = "Based on my analysis, here are the key differences and "
            ...
            response = "I understand your question and I'm happy to help with "
                "that."
            intent_response = "question"
...
async def integration_client(mock_model_manager, mock_cache_manager):
    """Create test client with properly injected mocked dependencies."""
    from asgi_lifespan import LifespanManager
    from httpx import ASGITransport, AsyncClient

    # Store original app state if it exists
    original_state = getattr(app.state, "app_state", None)
    try:
        ...
        mock_app_state = {
            ...
            "search_graph": AsyncMock(),
            "search_system": AsyncMock(),
            "startup_time": time.time(),
            "startup_errors": [],
            "api_key_status": {"brave_search": True, "scrapingbee": True},
        }
        # Inject mock state directly into app
        app.state.app_state = mock_app_state
        timeout = 3.0
        # Try to use lifespan, but fall back if it fails
        try:
            async with LifespanManager(
                app, startup_timeout=timeout, shutdown_timeout=timeout
            ):
            ...
            print(f"Warning: Lifespan setup failed, using basic client: {e}")
            async with AsyncClient(
                transport=ASGITransport(app=app), base_url="http://test"
            ) as client:
                yield client
        ...
        elif hasattr(app.state, "app_state"):
