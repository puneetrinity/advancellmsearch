...
class CacheManager:
    ...
    async def cache_successful_pattern(
        self, query: str, execution_path: List[str], cost: float
    ):
    ...
    async def deduct_budget(self, user_id: str, cost: float) -> float:
        ...
        current_remaining = budget_data.get(
            "remaining", self.settings.default_monthly_budget
        )
    ...
    async def check_rate_limit(
        ...
        try:
            if self.redis:
                ...
                rate_data = await self.get(rate_key) or {
                    "count": 0,
    ...
    async def cache_performance_hint(
        self, query_type: str, expected_time: float, expected_confidence: float
    ):
        ...
        if existing:
            ...
            hint_data["expected_time"] = (
                existing["expected_time"] * (sample_count - 1) + expected_time
            ) / sample_count
            hint_data["expected_confidence"] = (
                existing["expected_confidence"] * (sample_count - 1)
                + expected_confidence
            ) / sample_count
    ...
    async def cache_optimal_model(
        self, task_type: str, model_name: str, success_rate: float
    ):
    ...
    async def shutdown(self):
        """Shutdown the cache manager and close connections."""
        try:
