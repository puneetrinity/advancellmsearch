...
class IntentClassifierNode(BaseGraphNode):
    ...
    async def execute(self, state: GraphState, **kwargs) -> NodeResult:
        logger.debug(
            f"[IntentClassifierNode] Enter execute. state.query_id={getattr(state, 'query_id', None)}"
        )
        try:
            ...
            logger.debug(
                f"[IntentClassifierNode] Success. state.query_id={getattr(state, 'query_id', None)}"
            )
            return NodeResult(
                success=True,
                data={
...
class ResponseGeneratorNode(BaseGraphNode):
    ...
    async def execute(self, state: GraphState, **kwargs) -> NodeResult:
        logger.debug(
            f"[ResponseGeneratorNode] Enter execute. state.query_id={getattr(state, 'query_id', None)}"
        )
        try:
            ...
            if result.success:
                ...
                logger.debug(
                    f"[ResponseGeneratorNode] Success. state.query_id={getattr(state, 'query_id', None)}"
                )
                return NodeResult(
                    success=True,
                ...
                logger.warning(
                    f"[ResponseGeneratorNode] Model generation failed: {result.error}"
                )
                return NodeResult(
                    success=False,
    ...
    def _build_prompt(self, state: GraphState) -> str:
        ...
        query = getattr(state, "processed_query", None) or getattr(
...
class ChatGraph(BaseGraph):
    ...
    async def execute(self, state: GraphState) -> GraphState:
        logger.debug(
            f"[ChatGraph] Enter execute. state.query_id={getattr(state, 'query_id', None)}"
        )
        import time
