...
import asyncio
import logging
from typing import Any, Awaitable, TypeVar, Union, Callable, Optional
...
async def ensure_awaited(obj: Union[T, Awaitable[T]], max_depth: int = 10) -> T:
    ...
    if depth >= max_depth:
        logger.error(f"Max await depth ({max_depth}) exceeded - possible infinite coroutine chain")
        raise RuntimeError(f"Max await depth exceeded: {max_depth}")
    
    if asyncio.iscoroutine(obj):
        logger.warning("Object is still a coroutine after max depth - this should not happen")
...
class AsyncSafetyValidator:
    ...
    @staticmethod
    def assert_no_coroutines(response: Any, message: str = "Response contains coroutines"):
        """
