...
async def test_system_startup():
    ...
    for attempt in range(max_retries):
        ...
        print(f"  ⏳ Waiting for system startup (attempt {attempt + 1}/{max_retries})...")
...
async def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"  ✅ Metrics endpoint: {len(data['metrics']['components'])} components")
    except Exception as e:
...
async def test_chat_functionality():
    ...
    test_cases = [
        ...
        },
        {
            ...
            "expected_keywords": ["microservices", "monolithic", "advantage", "disadvantage"]
        },
        {
                        ...
                        if chunk.startswith("data: "):
                            ...
                            try:
                                ...
                                if "choices" in parsed_chunk and parsed_chunk["choices"]:
                                    content = parsed_chunk["choices"][0].get("delta", {}).get("content", "")
                                    if content:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if i > 0:  # After first message
                    ...
                    has_context = any(indicator in chat_response.lower() for indicator in context_indicators)
...
async def test_performance_under_load():
    ...
    print(f"  🏃 Running {total_requests} requests with {concurrent_requests} concurrent...")
...
async def test_system_metrics():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"     System uptime: {metrics.get('uptime', 0):.1f}s")
...
async def run_complete_system_demo():
    ...
    try:
        ...
        print(f"📊 Overall success rate: {successful_phases}/{total_phases} ({successful_phases/total_phases:.1%})")
        ...
        if critical_success:
            ...
            print(f"\n📝 EXAMPLE USAGE:")
            print(f"""   curl -X POST "http://localhost:8000/api/v1/chat/complete" \\
...
if __name__ == "__main__":
    ...
    parser.add_argument("--full", action="store_true", help="Run complete demo (default)")
    parser.add_argument("--wait", type=int, default=0, help="Wait N seconds before starting")
